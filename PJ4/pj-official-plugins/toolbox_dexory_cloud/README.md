...
./build.sh toolbox_dexory_cloud
```

Artifacts land under `build/toolbox_dexory_cloud/Release/`:

- `bin/libtoolbox_dexory_cloud_plugin.so` — the plugin (toolbox + borrowed dialog).
- `toolbox_dexory_cloud/dexory-cloud-cli` — the headless CLI (below).
...
(ixwebsocket transport; enforced by the `DexoryCloudCliNoQtGuard` ctest).
...
|---|---|---|---|
| `DEXORY_CLOUD_URL` | `--url URL` | `ws://localhost:8080` | WS base URI (`ws://` or `wss://`) |
| `DEXORY_CLOUD_API_KEY` | `--token TOKEN` | *(empty)* | bearer token; empty = dev anonymous |
...
`DexoryCloudCliUrlResolveTest` unit test (so `DEXORY_CLOUD_URL` is honored when
`--url` is absent, and `--url` overrides the env).

## Tests (ctest)

```bash
ctest --test-dir build/toolbox_dexory_cloud/Release            # hermetic: live tests SKIP
DEXORY_CLOUD_LIVE_URL=ws://localhost:8081 \
  ctest --test-dir build/toolbox_dexory_cloud/Release          # live: all run
```

Two modes, gated by `DEXORY_CLOUD_LIVE_URL`:
...
- **Hermetic** (default, no server): unit/header tests, wire-mapping and session
  ...
  (`DexoryCloudPluginLoadSmokeTest` — dlopens the built `.so` and asserts both
  the toolbox and dialog entry vtables resolve), the no-Qt guard, and the
  URL-resolution unit test. Live tests self-**skip**.
- **Live** (`DEXORY_CLOUD_LIVE_URL` set to a running server, e.g. the `make
