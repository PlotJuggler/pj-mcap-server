...
// on DEXORY_CLOUD_LIVE_URL so the hermetic suite skips them:
//
...
using namespace dexory_cloud;
using namespace dexory_cloud::testsupport;
...
// count, no duplicates.
TEST(DexoryCloudSessionResumeLive, ForcedMidPullDropResumesToComplete) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live resume test skipped";
  }
...
TEST(DexoryCloudSessionResumeLive, ResumeNotPossibleIsCleanVerbatim) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live resume test skipped";
  }
...
TEST(DexoryCloudSessionResumeLive, CancelDuringResumeYieldsCancelled) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live resume test skipped";
  }
...
TEST(DexoryCloudSessionResumeLive, RepeatFetchIsCacheHitZeroTransport) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live cache test skipped";
  }
...
TEST(DexoryCloudSessionResumeContract, BackoffScheduleIsSpec10) {
