...
using dexory_cloud::testsupport::FakeToolboxHost;
...
TEST(DexoryCloudParserIngestLive, FullPullPushesGroundTruthCounts) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live ingest test skipped";
  }
...
TEST(DexoryCloudParserIngestLive, CancelMidPullReleasesContext) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "DEXORY_CLOUD_LIVE_URL not set — live ingest test skipped";
  }
