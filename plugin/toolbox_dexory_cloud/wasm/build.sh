...
WASM_OUT="${BUILD_DIR}/dexory_cloud_wasm_smoke.js"
...
[ -s "${BUILD_DIR}/dexory_cloud_wasm_smoke.wasm" ] || fail ".wasm artifact missing"
WASM_BYTES="$(stat -c%s "${BUILD_DIR}/dexory_cloud_wasm_smoke.wasm" 2>/dev/null || echo '?')"
echo "Produced ${BUILD_DIR}/dexory_cloud_wasm_smoke.wasm (${WASM_BYTES} bytes)"
...
PB_GEN_DIR="$(find "${PLUGIN_DIR}/../build" -path '*toolbox_dexory_cloud/generated/pj_cloud.pb.h' 2>/dev/null \
