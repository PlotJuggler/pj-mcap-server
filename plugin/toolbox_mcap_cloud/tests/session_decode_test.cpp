...
//
// Registered as McapCloudSessionDecodeTest; runs in the hermetic CI suite.
...
TEST(McapCloudSessionDecode, Lz4SingletonRoundTrip) {
  ...
  *batch.add_messages() =
  ...
  std::vector<mcap_cloud::DecodedMessage> out;
  std::string error;
  ASSERT_TRUE(mcap_cloud::decodeBatch(batch, &out, &error)) << error;
...
TEST(McapCloudSessionDecode, Lz4TrailingGarbageRejected) {
  ...
  *batch.add_messages() = makeMessage(1, 1, 100, 100, wire, pj_cloud::v1::PAYLOAD_ENCODING_LZ4);
  ...
  std::vector<mcap_cloud::DecodedMessage> out;
  std::string error;
  EXPECT_FALSE(mcap_cloud::decodeBatch(batch, &out, &error))
...
TEST(McapCloudSessionDecode, Lz4TruncatedFrameRejected) {
...
TEST(McapCloudSessionDecode, ZstdBodyRoundTrip) {
  ...
  *body.add_messages() = makeMessage(1, 10, 3000, 3000, "");                      // empty payload
  ...
  std::vector<mcap_cloud::DecodedMessage> out;
  std::string err;
  ASSERT_TRUE(mcap_cloud::decodeBatch(batch, &out, &err)) << err;
...
TEST(McapCloudSessionDecode, ZstdBodySizeMismatchRejected) {
  pj_cloud::v1::MessageBatchBody body;
  *body.add_messages() = makeMessage(1, 0, 1, 1, "payload");
  ...
  std::vector<mcap_cloud::DecodedMessage> out;
  std::string err;
  EXPECT_FALSE(mcap_cloud::decodeBatch(batch, &out, &err));
  EXPECT_FALSE(err.empty());
}

// A corrupt ZSTD body (not a valid frame) is rejected, not misread.
TEST(McapCloudSessionDecode, ZstdBodyCorruptRejected) {
  pj_cloud::v1::MessageBatch batch;
  batch.set_body_encoding(pj_cloud::v1::BODY_ENCODING_ZSTD);
...
TEST(McapCloudSessionDecode, NoneSingletonRawAndZstd) {
...
TEST(McapCloudSessionDecode, UnspecifiedBodyTreatedAsNone) {
  pj_cloud::v1::MessageBatch batch;
...
TEST(McapCloudSessionDecode, UnknownBodyEncodingRejected) {
  pj_cloud::v1::MessageBatch batch;
...
TEST(DecodeEncodedEnvelope, RoundTripsExact) {
  ...
  std::string out;
  ASSERT_TRUE(mcap_cloud::decodeEncodedEnvelope(frame, big.size(), &out));
  EXPECT_EQ(out, big);
}
...
TEST(DecodeEncodedEnvelope, RejectsAnnouncedSizeMismatch) {
  ...
  std::string out;
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(frame, body.size() + 1, &out));
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(frame, body.size() - 1, &out));
}
...
TEST(DecodeEncodedEnvelope, RejectsOutOfBoundsAnnounced) {
  ...
  std::string out;
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(frame, 0, &out));
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(frame, mcap_cloud::kMaxDecodedEnvelopeBytes + 1, &out));
}
...
TEST(DecodeEncodedEnvelope, RejectsTrailingData) {
  ...
  std::string out;
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(frame, body.size(), &out));
}

// Garbage and empty bodies are rejected cleanly (no crash, no OOM).
TEST(DecodeEncodedEnvelope, RejectsGarbageAndEmpty) {
  std::string out;
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope("not a zstd frame at all", 32, &out));
  EXPECT_FALSE(mcap_cloud::decodeEncodedEnvelope(std::string{}, 32, &out));
}
