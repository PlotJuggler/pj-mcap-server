...
// (mcap_cloud_toolbox.cpp: PJ_TOOLBOX_PLUGIN + PJ_DIALOG_PLUGIN); the host's
...
// this test pins. The .so path arrives via the MCAP_PLUGIN_SO_PATH compile
...
TEST(McapCloudPluginLoad, ResolvesToolboxAndDialogVtables) {
  ...
  void* handle = dlopen(MCAP_PLUGIN_SO_PATH, RTLD_NOW | RTLD_LOCAL);
  ASSERT_NE(handle, nullptr) << "dlopen(" << MCAP_PLUGIN_SO_PATH << ") failed: " << dlerror();
