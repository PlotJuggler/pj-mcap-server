...
// Requires a running pj-cloud server; gated on MCAP_CLOUD_LIVE_URL so the
// hermetic CI suite skips it:
//
//   MCAP_CLOUD_LIVE_URL=ws://localhost:8080 ctest -R McapCloudBackendLive
//
...
const char* liveUrl() {
  return std::getenv("MCAP_CLOUD_LIVE_URL");
}
...
TEST(McapCloudBackendLive, ConnectListSequencesAndTopics) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "MCAP_CLOUD_LIVE_URL not set — live server test skipped";
  }

  mcap_cloud::BackendConnection conn(url, /*cert_path=*/"", /*api_key=*/"",
...
TEST(McapCloudBackendLive, BackendCapabilities) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "MCAP_CLOUD_LIVE_URL not set — live server test skipped";
  }
...
// "message_count" key.
TEST(McapCloudBackendLive, GroundTruthCatalog) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "MCAP_CLOUD_LIVE_URL not set — live server test skipped";
  }
  ...
  const mcap_cloud::SequenceInfo* known = nullptr;
  ...
  const mcap_cloud::TopicInfo* imu = nullptr;
  for (const auto& topic : topics) {
...
TEST(McapCloudBackendLive, StitchedGroundTruth) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "MCAP_CLOUD_LIVE_URL not set — live server test skipped";
  }
  ...
  const auto sequences = conn.listSequences();
  auto find = [&](const char* name) -> const mcap_cloud::SequenceInfo* {
    for (const auto& seq : sequences) {
      if (seq.name == name) {
        return &seq;
      }
  ...
  auto msgCount = [](const mcap_cloud::SequenceInfo& s) -> std::int64_t {
  ...
  const mcap_cloud::SequenceInfo* a = find(kStitchKeyA);
  const mcap_cloud::SequenceInfo* b = find(kStitchKeyB);
...
TEST(McapCloudBackendLive, UpdateTagsSetThenUnset) {
  const char* url = liveUrl();
  if (url == nullptr || *url == '\0') {
    GTEST_SKIP() << "MCAP_CLOUD_LIVE_URL not set — live server test skipped";
  }
  ...
  {
    std::vector<mcap_cloud::TagRow> tags;
