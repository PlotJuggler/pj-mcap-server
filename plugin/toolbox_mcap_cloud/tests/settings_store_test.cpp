...
struct Fixture {
  ...
  mcap_cloud::SettingsStore store() {
    return mcap_cloud::SettingsStore(PJ::sdk::SettingsView{host.view()});
  }
};
...
TEST(SettingsStore, McapSavePreferencesRoundTrip) {
  Fixture fx;
  auto store = fx.store();
  store.setBool("mcap_cloud/save_mcap", true);
  store.setString("mcap_cloud/save_directory", "/tmp/plotjuggler-mcaps");

  EXPECT_TRUE(fx.store().getBool("mcap_cloud/save_mcap", false));
  EXPECT_EQ(
      fx.store().getString("mcap_cloud/save_directory"),
...
// (mcap_cloud/selected_sequence + mcap_cloud/selected_topics) and restores them on
...
TEST(SettingsStore, SelectedSequenceAndTopicsRoundTrip) {
  ...
  store.setString("mcap_cloud/selected_sequence", "seq_2024_03");
  const std::vector<std::string> topics = {"/imu", "/camera/image", "/gps/fix"};
  store.setStringList("mcap_cloud/selected_topics", topics);

  // A fresh adapter over the same backend (mirrors a later session's
  // initFromSettings reading what persistState wrote).
  EXPECT_EQ(fx.store().getString("mcap_cloud/selected_sequence"), "seq_2024_03");
  EXPECT_EQ(fx.store().getStringList("mcap_cloud/selected_topics"), topics);
}
...
TEST(SettingsStore, SelectedSelectionDefaultsWhenAbsent) {
  ...
  Fixture fx;
  auto store = fx.store();
  EXPECT_EQ(store.getString("mcap_cloud/selected_sequence"), "");
  EXPECT_TRUE(store.getStringList("mcap_cloud/selected_topics").empty());
}
...
TEST(SettingsStore, UnboundViewReturnsDefaults) {
  ...
  mcap_cloud::SettingsStore store(PJ::sdk::SettingsView{});
