...
// configurable config root (production: $XDG_CONFIG_HOME/mcap_cloud); a
 ...
 protected:
  void SetUp() override {
    ...
    root_ = fs::temp_directory_path() /
            fs::path("globex_cred_test_" + std::to_string(::testing::UnitTest::GetInstance()->random_seed()) + "_" +
  ...
  mcap_cloud::FileCredentialStore store() { return mcap_cloud::FileCredentialStore(root_); }
...
TEST(CredentialResolve, EnvBeatsStored) {
  EXPECT_EQ(mcap_cloud::resolveStoredToken(/*env=*/std::string("from-env"),
                                             /*stored=*/std::optional<std::string>("from-store")),
            "from-env");
}

TEST(CredentialResolve, StoredUsedWhenNoEnv) {
  EXPECT_EQ(mcap_cloud::resolveStoredToken(/*env=*/std::nullopt,
                                             /*stored=*/std::optional<std::string>("from-store")),
            "from-store");
}
...
TEST(CredentialResolve, NoneWhenNeitherPresent) {
  EXPECT_EQ(mcap_cloud::resolveStoredToken(/*env=*/std::nullopt, /*stored=*/std::nullopt), "");
}
