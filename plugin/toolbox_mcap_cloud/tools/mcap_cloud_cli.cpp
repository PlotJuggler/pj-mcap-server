...
//
// mcap-cloud-cli — a headless command-line driver over the EXACT
...
void printUsage(std::ostream& os) {
  os << "Usage: mcap-cloud-cli [--url URL] [--token TOKEN] <command> [args]\n"
        "\n"
        "Commands:\n"
...
int runHello(mcap_cloud::BackendConnection& conn, const std::string& error) {
...
int runList(mcap_cloud::BackendConnection& conn, bool as_json) {
  ...
  for (const auto& seq : sequences) {
    std::cout << "\n- " << seq.name << '\n'
              ...
              << "    size:  " << mcap_cloud::formatBytes(seq.total_size_bytes) << " (" << seq.total_size_bytes
              << " bytes)\n";
    if (auto it = seq.user_metadata.find("message_count"); it != seq.user_metadata.end()) {
...
int runTopics(mcap_cloud::BackendConnection& conn, const std::string& sequence_name, bool as_json) {
  ...
  const std::vector<mcap_cloud::TopicInfo> topics = conn.listTopics(sequence_name);
  if (topics.empty()) {
  ...
  auto schemaField = [](const mcap_cloud::TopicInfo& t, const std::string& key) -> std::string {
...
const char* eosName(mcap_cloud::SessionEos eos) {
  switch (eos) {
    case mcap_cloud::SessionEos::Complete:
      return "COMPLETE";
    case mcap_cloud::SessionEos::Cancelled:
      return "CANCELLED";
    case mcap_cloud::SessionEos::Error:
      return "ERROR";
    case mcap_cloud::SessionEos::Unset:
    default:
      return "UNSET";
  }
}
...
int runDownload(mcap_cloud::BackendConnection& conn, const std::vector<std::string>& sequence_names,
  ...
  if (const std::string overlap = mcap_cloud::validateNonOverlapping(sel_inputs); !overlap.empty()) {
    std::cerr << "download: cannot stitch (" << overlap << ")\n";
    return kExitFailure;
  }

  const mcap_cloud::StitchedSelection stitched = mcap_cloud::buildStitchedSelection(sel_inputs);
...
bool resolveStitchedParams(mcap_cloud::BackendConnection& conn, const std::vector<std::string>& sequence_names,
                           const std::vector<std::string>& topics, const std::optional<std::int64_t>& start_ns,
                           const std::optional<std::int64_t>& end_ns, mcap_cloud::OpenSessionParams* params,
...
int runDebug(mcap_cloud::BackendConnection& conn, const std::vector<std::string>& sequence_names,
             const std::vector<std::string>& topics, const std::optional<std::int64_t>& start_ns,
  ...
  auto on_message = [&](const mcap_cloud::DecodedMessage& msg) -> bool {
  ...
  const bool clean_abort = stats.eos == mcap_cloud::SessionEos::Cancelled && stats.error == "download aborted by sink";
...
int runTag(mcap_cloud::BackendConnection& conn, const std::string& sequence_name,
  ...
  std::vector<mcap_cloud::TagRow> effective;
  std::string error;
