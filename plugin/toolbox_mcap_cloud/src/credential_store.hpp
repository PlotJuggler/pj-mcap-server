...
// is compiled ONLY into the plugin .so, never into mcap-cloud-cli, so the
...
class FileCredentialStore : public CredentialStore {
 public:
  ...
  // production this is defaultConfigRoot() ($XDG_CONFIG_HOME/mcap_cloud or
  // $HOME/.config/mcap_cloud); tests inject a private temp dir.
...
//   $XDG_CONFIG_HOME/mcap_cloud, else $HOME/.config/mcap_cloud, else
//   <temp>/mcap_cloud (never empty). Credentials are CONFIG (not data), so
