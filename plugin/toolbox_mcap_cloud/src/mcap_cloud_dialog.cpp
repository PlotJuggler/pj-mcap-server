...
// Distinct, sorted values recorded for `key` across the dataset. The schema is
// built by buildCanonicalSchema (sorted-unique per key — the Schema type
// contract), so this is a plain lookup + copy, not a per-call sort.
std::vector<std::string> schemaValues(const Schema& schema, const std::string& key) {
  auto it = schema.find(key);
  return it == schema.end() ? std::vector<std::string>{} : it->second;
}

// Reference into the schema's value vector for `key` (empty vector when the
// key has no values). For per-call consumers that must not copy.
...
std::string sessionLabel(const mcap_cloud::AggSession& s) {
  std::string robot;
  std::string source;
...
// kBasicFilterKeys (the full 4-field canonical set), canonicalFilterFields and
// canonicalVocabularyKeys moved to canonical_fields.h, next to
// buildCanonicalSchema — the one-pass sorted-unique vocabulary that
// ensureQuerySchemaLocked caches per seq_epoch.
...
// (distinctFieldValues was RETIRED here: re-parsing every S3 key per dropdown
// per widget_data() call cost ~1.3 ms per 1000 files per call at the 20 Hz tick
// plus every forwarded widget event — dropdown values now come from the
// epoch-cached query_schema via schemaValuesRef.)
...
// (canonicalFilterFields / canonicalVocabularyKeys live in canonical_fields.h.
// The canonical fields are the ONLY filter dimension: the Advanced (Lua) tab
// evaluates against this map too, NOT against rec.metadata (which carries
// MCAP-content stats like chunk_count/message_count/duration for DISPLAY +
// stitching only). A query referencing any other key resolves to absent, so
// MCAP contents can never be a filter — only the dedicated server-derived key
// fields.)
...
McapCloudDialog::McapCloudDialog() : worker_(std::make_unique<FetchWorker>()) {
  worker_->connectFinished = [this](bool ok, std::string uri, std::string status, std::string err) {
    postEvent([this, ok, uri = std::move(uri), status = std::move(status), err = std::move(err)]() mutable {
      onConnectFinished(ok, std::move(uri), std::move(status), std::move(err));
    });
  };
  ...
  worker_->vocabularyFailed = [this](std::uint64_t request_id) {
    postEvent([this, request_id] { onVocabularyFailed(request_id); });
  };
  worker_->gatePageReady = [this](std::uint64_t request_id, std::vector<SequenceInfo> page, bool reset) {
    postEvent([this, request_id, page = std::move(page), reset]() mutable {
      onGatePageReady(request_id, std::move(page), reset);
    });
  };
...
void McapCloudDialog::setHostProvider(std::function<PJ::sdk::ToolboxHostView()> provider) {
...
void McapCloudDialog::initFromSettings() {
  ...
  state_.range_upper =
      std::clamp(settings.getInt("mcap_cloud/range_upper", DialogState::kSliderSteps), 0, DialogState::kSliderSteps);
  ...
  for (const auto& [key, label] : kLocalBasicFilterKeys) {
    (void)label;
    const std::string value = settings.getString(std::string("mcap_cloud/basic_filter/") + key);
    if (!value.empty()) {
...
McapCloudDialog::~McapCloudDialog() {
...
void McapCloudDialog::workerLoop() {
  for (;;) {
...
void McapCloudDialog::postCommand(std::function<void()> fn) {
  {
    std::lock_guard<std::mutex> lock(cmd_mu_);
...
void McapCloudDialog::postEvent(std::function<void()> fn) {
  std::lock_guard<std::mutex> lock(evt_mu_);
  evt_queue_.push_back(std::move(fn));
}

bool McapCloudDialog::onTick() {
...
std::string McapCloudDialog::manifest() const {
  return kMcapCloudPanelManifest;
}

std::string McapCloudDialog::ui_content() const {
  return kMcapCloudPanelUi;
}

std::string McapCloudDialog::saveConfig() const {
...
bool McapCloudDialog::loadConfig(std::string_view config_json) {
...
std::string McapCloudDialog::widget_data() {
  ...
  // PJ3 parity: combo always lists the MRU history + the default server pin.
  {
    ...
    if (!state_.server_history_loaded) {
      state_.server_history = SettingsStore(settings_).getStringList("mcap_cloud/server_history");
      state_.server_history_loaded = true;
    }
    const std::vector<std::string>& history = state_.server_history;
  ...
  // each eval and cleared after.
  {
    ...
    if (!cache_hit) {
      ...
      if (state_.aggregate) {
        ...
        std::vector<mcap_cloud::AggInput> inputs;
        inputs.reserve(state_.sequences.size());
        for (const auto& rec : state_.sequences) {
          inputs.push_back({rec.name, rec.min_ts_ns, rec.max_ts_ns});
        }
        const auto sessions = mcap_cloud::aggregateSessions(inputs, kSessionGapThresholdNs);
    ...
    // Dropdown values come from the epoch-cached canonical schema (already
    // refreshed by the assist block's ensureQuerySchemaLocked above) — never
    // from a per-call sweep over the sequences.
    for (const auto& [key, label] : kLocalBasicFilterKeys) {
  ...
  // Browse-gate pill: an overlay on top of seqTable (same grid cell,
  // seqTableStack) explaining why the table is empty — disconnected, loading,
  ...
  if (state_.open_cert_pending) {
    ...
    // the *saved* values, not the MCAP_CLOUD_API_KEY env
...
bool McapCloudDialog::onTextChanged(std::string_view widget_name, std::string_view text) {
  std::lock_guard<std::mutex> lock(state_.mu);
...
bool McapCloudDialog::onClicked(std::string_view widget_name) {
  ...
  if (widget_name == "buttonRefresh") {
    // Re-list without a disconnect/reconnect (PJ3 onRefreshClicked,
    // main_window.cpp:933-945). No-op unless connected and idle. The catalog is
...
bool McapCloudDialog::onTabChanged(std::string_view /*widget_name*/, int /*index*/) {
  return false;  // no tab widgets left; the filter mode is the radio pair above
}

bool McapCloudDialog::onValueChanged(std::string_view /*widget_name*/, int /*value*/) {
  return false;
}

bool McapCloudDialog::onRangeChanged(std::string_view widget_name, int lower, int upper) {
...
bool McapCloudDialog::onDateRangeChanged(
...
std::string McapCloudDialog::fetchStatusLineLocked() const {
...
void McapCloudDialog::recomputeTopicUnionLocked() {
...
void McapCloudDialog::ensureQuerySchemaLocked() {
  ...
  // without a per-call re-sort or key re-parse.
  state_.query_schema = buildCanonicalSchema(state_.sequences);
  state_.query_schema_epoch = state_.seq_epoch;
}

bool McapCloudDialog::onCodeChangedWithCursor(std::string_view widget_name, std::string_view code, int cursor) {
  if (widget_name != "lua_queryBar") {
    return false;
  }
  std::lock_guard<std::mutex> lock(state_.mu);
...
bool McapCloudDialog::onIndexChanged(std::string_view widget_name, int index) {
  if (index < 0) {
    return false;
  }
  // Browse-gate combos (customer/site): unlike the Basic-tab dropdowns below,
  // these carry NO "(any)" entry — index resolves DIRECTLY against the
  // vocabulary lists getWidgetData() populated (customers in vocabulary order;
  // sites via siteNamesFor(gate_customer)). Both guard against an
  // out-of-range index (the vocabulary can change under a stale combo echo).
  if (widget_name == "filter_customer") {
    std::string new_customer;
    std::string server_key;
    {
      ...
      new_customer = state_.vocabulary->customers[static_cast<std::size_t>(index)].name;
  ...
  if (widget_name == "filter_customer_site") {
    ...
    std::uint64_t id = 0;
    {
      ...
      const std::vector<std::string> sites = siteNamesFor(*state_.vocabulary, state_.gate_customer);
      if (index >= static_cast<int>(sites.size())) {
        return true;  // out of range -- vocabulary changed under us; ignore
      }
      new_site = sites[static_cast<std::size_t>(index)];
      if (new_site == state_.gate_site) {
        return true;  // no-op re-pick
      }
...
std::uint64_t McapCloudDialog::beginGateRequestLocked(GatePhase next_phase) {
  ...
  // hide — the old date span would mask the new site's progressive pages) the
  ...
  // (date_filter.h) treats 0/0 as unbounded, so a stale span picked for the
...
void McapCloudDialog::onConnectFinished(bool ok, std::string uri, std::string status, std::string error) {
  // `uri` is the URI THIS connectAsync() call actually targeted (threaded
  ...
  if (ok) {
    ...
    // (cap 20) and persist via the settings store ("mcap_cloud/server_history").
    SettingsStore settings(settings_);
    ...
    // gate's data source) instead of the old unconditional listSequencesAsync.
    // onVocabularyReady resolves/auto-selects a customer+site (persisted
    // per-server, with a one-shot migration off the legacy global Basic-tab
    // keys) and starts the gated sweep itself.
    postCommand([w = worker_.get(), gate_request_id] { w->fetchVocabularyAsync(gate_request_id); });
    return;
  }
...
void McapCloudDialog::onVocabularyReady(std::uint64_t request_id, VocabularyInfo vocab, bool recovery) {
  ...
  std::uint64_t id = 0;
  {
    ...
    if (customer.empty()) {
      customer = autoSelectCustomer(vocab);
    }
    // ONE-SHOT MIGRATION: nothing above resolved a full selection — seed from
    // the legacy GLOBAL (pre-gate) Basic-tab keys this build no longer writes,
    // then clear them so a downgrade can't resurrect a value from an older
    // install that predates the per-server gate keys.
    if (customer.empty() || site.empty()) {
      SettingsStore settings(settings_);
      bool migrated = false;
      if (customer.empty()) {
        const std::string legacy = settings.getString("mcap_cloud/basic_filter/customer");
        if (!legacy.empty()) {
          customer = legacy;
          migrated = true;
        }
      ...
      if (site.empty()) {
        const std::string legacy = settings.getString("mcap_cloud/basic_filter/customer_site");
        if (!legacy.empty()) {
          site = legacy;
          migrated = true;
        }
      ...
      if (migrated) {
        settings.remove("mcap_cloud/basic_filter/customer");
        settings.remove("mcap_cloud/basic_filter/customer_site");
      }
    }
...
void McapCloudDialog::onGateListFinished(FetchWorker::GateListResult result) {
  // May arrive WITHOUT a preceding dialog-initiated transition (the tag-edit
  // re-list path reuses the stored gate id) — gate on request_id ALONE, never
  // on an assumed prior phase.
  using Err = FetchWorker::GateListResult::Error;
  bool complete = false;
  ...
  std::string warning;
  {
    ...
    switch (result.error) {
      case Err::kSuperseded:
        return;  // a newer request already owns the UI; nothing to show
      case Err::kNone:
        complete = true;
        break;
      case Err::kSelectionGone: {
        // The (customer, site) pair no longer resolves against the current
        // vocabulary (a rebuild renamed/removed the site out from under an
        // in-flight sweep). Keep the customer, drop the site, and send the
      ...
      case Err::kRebuildStorm:
        state_.gate_phase = GatePhase::kListError;
        notify_warning = true;
        warning = "Catalog is rebuilding faster than it can be listed — use Refresh to retry";
        break;
    }
  }

  if (complete) {
    // NEVER render "no recordings" for anything but a genuinely complete,
    // empty result — this IS that case. Reuse the existing authoritative
    // onSequencesReady path (date seeding, stale-value pruning, reselect,
    // the "N sequences" notify) exactly as the legacy unfiltered path did.
    const bool empty = result.sequences.empty();
    onSequencesReady(std::move(result.sequences));
    std::lock_guard<std::mutex> lock(state_.mu);
    // A newer gate request could have superseded us WHILE onSequencesReady ran
    // (it re-locks internally); only the request that is STILL current gets to
    // set the terminal phase.
    if (result.request_id == state_.gate_request_seq) {
      state_.gate_phase = empty ? GatePhase::kListEmpty : GatePhase::kRows;
    }
    return;
  }
  if (notify_warning) {
    notify(PJ::ToolboxMessageLevel::kWarning, warning);
  }
}

void McapCloudDialog::onPullPhase(std::string phase) {
  std::lock_guard<std::mutex> lock(state_.mu);
...
void McapCloudDialog::onPullProgress(std::string topic_name, std::int64_t bytes) {
  std::lock_guard<std::mutex> lock(state_.mu);
...
void McapCloudDialog::onPullFinished(
...
void McapCloudDialog::onAllFetchesComplete(std::string sequence_name) {
  ...
  if (!summary.error_summary.empty()) {
    notify(PJ::ToolboxMessageLevel::kError, fmt::format("MCAP Cloud fetch errors:\n{}", summary.error_summary));
  }
...
void McapCloudDialog::onPullResuming(std::string /*group*/, unsigned attempt, unsigned max) {
...
void McapCloudDialog::onPullServedFromCache(std::string group) {
  notify(PJ::ToolboxMessageLevel::kInfo, fmt::format("Served from cache: {}", group));
}

void McapCloudDialog::persistState() {
  std::string query;
  int lower = 0;
  ...
  SettingsStore settings(settings_);
  settings.setString("mcap_cloud/metadata_query", query);
  settings.setInt("mcap_cloud/range_lower", lower);
  settings.setInt("mcap_cloud/range_upper", upper);
  settings.setString("mcap_cloud/selected_sequence", selected_sequence);
  ...
  // here); an unset filter writes "" so a previously persisted constraint is
  // cleared, not resurrected.
  for (const auto& [key, label] : kLocalBasicFilterKeys) {
    (void)label;
    const auto it = basic_filter.find(key);
    settings.setString(
        std::string("mcap_cloud/basic_filter/") + key, it != basic_filter.end() ? it->second : std::string{});
  }
}

void McapCloudDialog::notify(PJ::ToolboxMessageLevel level, const std::string& message) {
...
void McapCloudDialog::rebuildSeqDisplayLocked() {
...
void McapCloudDialog::sortSequencesLocked() {
...
void McapCloudDialog::sortTopicsLocked() {
...
void McapCloudDialog::onTagsUpdated(std::string sequence_name, bool ok, std::string error) {
...
void McapCloudDialog::onTopicsFailed(std::string sequence_name, std::string error) {
  ...
  std::string message;
  {
    ...
    message = fmt::format("MCAP Cloud: topics request failed: {}", error);
    recomputeTopicUnionLocked();
  }
  if (notify_user) {
...
void McapCloudDialog::onConnectionLost() {
  {
    std::lock_guard<std::mutex> lock(state_.mu);
    if (!state_.connected) {
    ...
    // Leave rows on screen (cached from before the drop); the pill only shows
    // when state_.sequences is empty, so a populated table stays visible.
    state_.gate_phase = GatePhase::kDisconnected;
  }
  notify(PJ::ToolboxMessageLevel::kWarning, "MCAP Cloud: connection to the server was lost — press Connect to reconnect");
}

void McapCloudDialog::onTopicMetadataReady(std::string sequence_name, std::string topic_name, TopicInfo info) {
  std::lock_guard<std::mutex> lock(state_.mu);
