...
```bash
cd PJ4/pj-official-plugins/toolbox_mcap_cloud/wasm
...
plugin `.so`, the `mcap-cloud-cli`, the ctest suite (incl. `McapCloudSessionDecodeTest`),
`make smoke`, and `make matrix` are **byte-for-byte behaviorally unchanged** (verified:
native `./build.sh toolbox_mcap_cloud` green under full `-Werror`, ctest 31/31 hermetic,
`SMOKE PASS`).
