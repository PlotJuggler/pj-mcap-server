...
void testSessionCache() {
  using namespace PJ::cloud;
  using mcap_cloud::CachedSession;
...
void testStitch() {
  using mcap_cloud::buildStitchedSelection;
  using mcap_cloud::SelInput;
  using mcap_cloud::validateNonOverlapping;
...
void testZstdDecode() {
  ...
  std::string frame(reinterpret_cast<const char*>(mcap_cloud_wasm_smoke::kZstdFrame),
                    mcap_cloud_wasm_smoke::kZstdFrame_len);
  std::string expected(reinterpret_cast<const char*>(mcap_cloud_wasm_smoke::kPlaintext),
                       mcap_cloud_wasm_smoke::kPlaintext_len);
  std::string out;
  std::string error;
  ...
  check(out.size() == mcap_cloud_wasm_smoke::kPlaintext_len, "zstd uncompressed-size-match");
...
int main() {
  std::printf("mcap-cloud wasm decode-core smoke (zstd %d.%d.%d decoder)\n", ZSTD_VERSION_MAJOR,
