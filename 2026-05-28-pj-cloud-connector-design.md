...
**Deferred whole-file fast paths (post-M1, not in v1).** Two known optimizations apply *only* to the unfiltered "download an entire run — all topics, full range" case, where no server-side subsetting is needed: (a) **MCAP chunk passthrough** — forward already-compressed source chunks verbatim (zero server recompression); and (b) **presigned-URL direct read** — hand the client a short-lived URL so it fetches bytes straight from the bucket (zero server bandwidth), the pattern [lakeFS](https://github.com/treeverse/lakefs) uses via `block.Adapter.GetPreSignedURL`. Both are **rejected for the v1 filtered-streaming path**: neither can honor a topic/time subset (a chunk is an atomic compression unit; a presigned URL serves whole objects); a presigned URL is intrinsically storage-specific and would break the storage-agnostic wire protocol and the single-codepath client; and direct bucket access widens the auth boundary beyond the single server-owned identity. Revisit only if profiling shows full-run replays dominate and server bandwidth (not client decode) is the bottleneck.
