...
- [ ] **2.5** New wire filters: extend `FileFilter` (proto) with dimension + `has_error` predicates → regenerate Go bindings (checked-in) → client (`toolbox_mcap_cloud`) gains the facet UI. **DECISION D3:** do this now, or hide dimensions behind existing tag/metadata semantics for M1 and add native filters later.
...
- [ ] **6.4** `run.sh` / bring-up: `--local` etc. must now (a) start the Python builder against the bucket and (b) start the Go server read-only. Preserve the one-command local bring-up.
